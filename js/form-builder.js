...
(function ($) {
  ...
  var FormBuilder = function FormBuilder(element, options) {
    ...
    _helpers.save = function () {
      ...
      $sortableFields2.children('li').not('.disabled').each(function () {
        _helpers.updatePreview($(this));
      });
    ...
    // Sortable fields 2
    var frmbID2 = 'frmb-' + ($('ul[id^=frmb-]').length+1);
    var $sortableFields2 = $('<ul/>').attr('id', frmbID2).addClass('frmb').sortable({
      cursor: 'move',
      opacity: 0.9,
      beforeStop: function beforeStop(event, ui) {
        var lastIndex = $('> li', $sortableFields2).length - 1,
            curIndex = ui.placeholder.index();
        doCancel = curIndex <= 1 || curIndex === lastIndex;
      },
      //start: _helpers.startMoving,
      ...
      placeholder: 'frmb-placeholder2'
    });

    $sortableFields.sortable( "option", "connectWith", $sortableFields2);
    $sortableFields2.sortable( "option", "connectWith", $sortableFields);

    // ControlBox with different fields
    cbUL.sortable({
      helper: 'clone',
      opacity: 0.9,
      connectWith: [$sortableFields, $sortableFields2],
      cursor: 'move',
      ...
      update: function update(event, ui) {
        // _helpers.stopMoving;
        elem.stopIndexCol1 = $('li', $sortableFields).index(ui.item) === 0 ? '0' : $('li', $sortableFields).index(ui.item);
        var targetCol = $sortableFields;
        var col = 1;
        if(elem.stopIndexCol1 == -1){
          elem.stopIndexCol2 = $('li', $sortableFields2).index(ui.item) === 0 ? '0' : $('li', $sortableFields2).index(ui.item);
          targetCol = $sortableFields2;
          col = 2;
        }
        if ($('li', targetCol).index(ui.item) < 0) {
          $(this).sortable('cancel');
        } else {
          prepFieldVars($(ui.item[0]), true, col);
        }
      },
      receive: function receive(event, ui) {
    ...
    elem.before($sortableFields).parent().prepend(frmbHeader).addClass('frmb-wrap col-md-10').append(actionLinks, viewXML, saveAll);
    elem.before($sortableFields2);

    $(".frmb").css({float: 'left', width: '48%'});

    var cbWrap = $('<div/>', {
      id: frmbID + '-cb-wrap',
      'class': 'cb-wrap  col-md-2'
    ...
    $('input, select', $sortableFields2).on('change', doSave);
    $('input, select', $sortableFields2).on('blur', doSave);
    ...
    var prepFieldVars = function prepFieldVars($field, isNew, col) {
      isNew = isNew || false;
    ...
    // add number dropdown
    var appendNumbers = function appendNumbers(values) {
    ...
    // append file field
    var appendDocument = function(values) {
      appendFieldLi(opts.messages.text, advFields(values), values);
    };

    var appendSelectList2 = function appendSelectList(values) {
    ...
    var appendNewField = function appendNewField(values) {
      ...
      var appendFieldType = {
        ...
        'radio-group-yes-no-na': appendSelectList2,
    ...
    var appendFieldLi = function appendFieldLi(title, field, values) {
      ...
      var li = '',
          delBtn = '<a id="del_' + lastID + '" class="del-button delete-confirm" href="#" title="' + opts.messages.removeMessage + '">' + opts.messages.remove + '</a>',
          toggleBtn = '<a id="frm-' + lastID + '" class="toggle-form icon-pencil" href="#" title="' + opts.messages.hide + '"></a> ',
      ...
      if(values.col == 1){
        if (elem.stopIndexCol1) {
          $('li', $sortableFields).eq(elem.stopIndexCol1).after(li);
        } else {
          $sortableFields.append(li);
        }
      }else{
        if (elem.stopIndexCol2) {
          $('li', $sortableFields2).eq(elem.stopIndexCol2).after(li);
        } else {
          $sortableFields2.append(li);
        }
      }
      ...
      switch (attrs.type) {
        ...
        case 'select':
          ...
          for (i = attrs.values.length - 1; i >= 0; i--) {
            ...
            options += '<option value="' + attrs.values[i].value + '">' + attrs.values[i].value + '</option>';
          }
        ...
        case 'autocomplete':
          ...
          break;
        case 'radio-group-yes-no-na':
              preview = '<input type="radio">Yes';
              preview += ' <input type="radio">No';
              preview += ' <input type="radio">N/A';
              break;
        case 'numbers':
              var numbers = "";
              for(var i=0; i<= 100; i++){
                numbers += "<option value='-1'>"+i+"</option>";
              }
              preview = "<select class='no-drag form-control'>"+numbers+"</select>";
              break;
        case 'document':
              preview = "<input type=file>"; break;
        default:
          preview = '<' + attrs.type + '></' + attrs.type + '>';
      }
    ...
    // delete options
    $sortableFields.add($sortableFields2).delegate('.remove', 'click', function (e) {
      e.preventDefault();
    ...
    // toggle fields
    $sortableFields.add($sortableFields2).on('click', '.toggle-form', function (e) {
      e.preventDefault();
    ...
    // update preview to label
    $sortableFields.add($sortableFields2).delegate('input[name="label"]', 'keyup', function () {
    ...
    // Delete field
    $sortableFields.add($sortableFields2).delegate('.delete-confirm', 'click', function (e) {
      e.preventDefault();
      ...
      if (opts.showWarning) {
        ...
        $field.slideUp(250, function () {
          $(this).remove();
          _helpers.save();
          if ($('.form-field', $sortableFields).length === 0 && $('.form-field', $sortableFields2).length === 0) {
    ...
    $sortableFields.add($sortableFields2).delegate('input.required', 'click', function () {
    ...
    $sortableFields.add($sortableFields2).delegate('.add_ck', 'click', function () {
    ...
    $sortableFields.add($sortableFields2).delegate('.add_opt', 'click', function (e) {
      e.preventDefault();
    ...
    $sortableFields.add($sortableFields2).delegate('.close_field', 'click', function (e) {
      e.preventDefault();
      $(this).parents('li.form-field').find('.toggle-form').trigger('click');
    });

    // Attach a callback to add new radio fields
    $sortableFields.add($sortableFields2).delegate('.add_rd', 'click', function (e) {
      e.preventDefault();
    ...
    /*if editing, we have to add the existing form fields*/
    if(current_fields != undefined && current_fields.length != 0){
      for(var i2 = 0; i2< current_fields.length; i2++){
        var val = {};
        val.label = _helpers.htmlEncode(current_fields[i2].title);
        val.required = (current_fields[i2].required == "1") ? 'true' : 'false';
        val.type = current_fields[i2].type;
        val.col = current_fields[i2].column;
        val.id = current_fields[i2].id;
        val.order = current_fields[i2].order;

          if(current_fields[i2].select_options != undefined){

            val.multiple = true;
            val.values = [];

            for(var j = 0; j< current_fields[i2].select_options.length; j++){
              var value = {
                label: current_fields[i2].select_options[j],
                value: current_fields[i2].select_options[j]
              };
...
(function ($) {
  ...
  $.fn.toXML = function (options) {
    ...
    this.each(function () {
      ...
      var col = ($(this).attr('id') == "frmb-0") ? 1 : 2;
      ...
      if ($(this).children().length >= 1) {
        ...
        $(this).children().each(function () {
          ...
          if (!($field.hasClass('moving') || $field.hasClass('disabled'))) {
            var attr = {};
            attr.required = $('input.required', $field).is(':checked') ? true : false;
            attr.type = $field.attr('class').replace(' form-field', '');
            attr.label = $('input.fld-label', $field).val();
            if(attr.type.match(/(select|checkbox-group|radio-group)/)){
              attr.options = [];
              $('.sortable-options li', $field).each(function () {
                var $option = $(this);

                attr.options.push($('.option-value', $option).val());
              });
            }
            attr.col = col;
            attr.order = order++;
            form_fields.push(attr);

          }
        });
      } // if "$(this).children().length >= 1"
    });
    return JSON.stringify(form_fields);
  };
})(jQuery);